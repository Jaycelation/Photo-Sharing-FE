...
function TopBar({ context, user, setUser, setContext }) {
    ...
    const handleLogout = async () => {
        try {
            await fetch(`${BE_URL}/admin/logout`, { method: 'POST', credentials: 'include' })
            setUser(null)
            setContext("Home")
    ...
    return (
        <>
            ...
            >
                <DialogTitle>
                    Search Results ({searchType === 'user' ? 'Users' : 'Photos with Comments'})
                </DialogTitle>
                <DialogContent>
                    <List>
                        {searchResults.length > 0 ? (
                            searchResults.map((item) => (
                                <div key={item._id}>
                                    {searchType === 'user' ? (
                                        <ListItem button component={Link} to={`/users/${item._id}`} onClick={() => setOpenDialog(false)}>
                                            <ListItemAvatar>
                                        ...
                                        <ListItem button component={Link} to={`/photos/${item.user_id._id || item.user_id}`} onClick={() => setOpenDialog(false)}>
                                            <ListItemAvatar>
                                                <Avatar variant="square" src={`${BE_URL}/images/${item.file_name}`} />
                                            </ListItemAvatar>
                                            <ListItemText
                                                primary={
                            ...
                            <Typography p={2} align="center">No results found.</Typography>
                        )}
                    </List>
                </DialogContent>
            </Dialog>
